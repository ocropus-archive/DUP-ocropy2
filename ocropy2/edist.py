...
def xlevenshtein(a,b,context=1, SKP="_", ERR="\003"):
    ...
    assert ERR not in al and ERR not in bl
    same = array([al[i]==bl[i] for i in range(len(al))],'i')
    same = filters.minimum_filter(same,1+2*context)
    als = "".join([al[i] if not same[i] else ERR for i in range(len(al))])
    bls = "".join([bl[i] if not same[i] else ERR for i in range(len(bl))])
    # print(als)
    # print(bls)
    ags = re.split(ERR+r'+',als)
    bgs = re.split(ERR+r'+',bls)
